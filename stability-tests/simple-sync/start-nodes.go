...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("dmgcoin-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("dmgcoin-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("DMGCOIN-SYNCER",
		"dmgcoin",
	...
	syncedCmd, err := common.StartCmd("DMGCOIN-SYNCED",
		"dmgcoin",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
