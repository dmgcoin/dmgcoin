...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("dmgcoinsanity-dmgcoin-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("dmgcoin", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "DMGCOIN-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "DMGCOIN-STDERR")
