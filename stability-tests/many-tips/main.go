...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("dmgcoin-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("dmgcoin datadir: %s", dataDir)

	dmgcoinCmd, err := common.StartCmd("DMGCOIN",
		"dmgcoin",
	...
	spawn("startNode-dmgcoinCmd.Wait", func() {
		err := dmgcoinCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("dmgcoinCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes dmgcoin closes ungracefully
				log.Errorf("dmgcoinCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(dmgcoinCmd, "dmgcoinCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	dmgcoinMinerCmd, err := common.StartCmd("MINER",
		"dmgcoinminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("dmgcoin-miner-Cmd.Wait", func() {
		err := dmgcoinMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(dmgcoinMinerCmd, "dmgcoinMinerCmd")
	return nil
}
