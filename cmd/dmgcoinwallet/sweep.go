...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libdmgcoinwallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libdmgcoinwallet.DmgcoinwalletdUTXOsTolibdmgcoinwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatDmgc(splitTransactions[i].Outputs[0].Value), " DMGC")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatDmgc(totalExtracted), " DMGC")
