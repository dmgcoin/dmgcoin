...
// Takes in a string representation of the Dmgc value to convert to Sompi
func TestDmgcToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := DmgcToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := DmgcToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateDMGCAmountFormat(testCase)
