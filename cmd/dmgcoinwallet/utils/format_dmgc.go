...
// FormatDmgc takes the amount of sompis as uint64, and returns amount of DMGC with 8  decimal places
func FormatDmgc(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerDmgcoin)
	}
	return res
}

// DmgcToSompi takes in a string representation of the DMGC value to convert to Sompi
func DmgcToSompi(amount string) (uint64, error) {
	err := validateDMGCAmountFormat(amount)
	...
	if constants.SompiPerDmgcoin%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerDmgcoin is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerDmgcoin))
	decimalStr := ""
...
func validateDMGCAmountFormat(amount string) error {
