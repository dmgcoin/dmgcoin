...
import (
	...
	"github.com/dmgcoin/dmgcoin/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		dmgcoinMessage, err := client.Post(&protowire.DmgcoinMessage{Payload: &protowire.DmgcoinMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := dmgcoinMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	dmgcoinMessage := &protowire.DmgcoinMessage{}
	err := protojson.Unmarshal([]byte(response), dmgcoinMessage)
	if err != nil {
	...
	return marshalOptions.Format(dmgcoinMessage)
}
