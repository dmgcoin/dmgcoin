...
// 2. A wrapper that implements isDmgcoinMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `DmgcoinMessage_` prefix and `Request` suffix
...
// generateDmgcoinMessage generates a wrapped DmgcoinMessage with the given `commandValue`
func generateDmgcoinMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.DmgcoinMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	dmgcoinMessage := reflect.New(reflect.TypeOf(protowire.DmgcoinMessage{}))
	dmgcoinMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return dmgcoinMessage.Interface().(*protowire.DmgcoinMessage), nil
}
