...
import (
	...
	"github.com/dmgcoin/dmgcoin/infrastructure/os/execenv"
	"github.com/dmgcoin/dmgcoin/infrastructure/os/limits"
	"github.com/dmgcoin/dmgcoin/infrastructure/os/signal"
	"github.com/dmgcoin/dmgcoin/infrastructure/os/winservice"
	"github.com/dmgcoin/dmgcoin/util/panics"
	"github.com/dmgcoin/dmgcoin/util/profiling"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "dmgcoinsvc",
	DisplayName: "DMGCoin Service",
	Description: "Downloads and stays synchronized with the DMGCoin blockDAG and " +
		"provides DAG services to applications.",
}

type dmgcoinApp struct {
	cfg *config.Config
}

// StartApp starts the dmgcoin app, and blocks until it finishes running
func StartApp() error {
...
func (app *dmgcoinApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start dmgcoin: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down dmgcoin...")
		...
		log.Infof("Dmgcoin shutdown complete")
	}()
