...
// Outpoint defines a dmgcoin data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new dmgcoin transaction outpoint point with the
...
// TxIn defines a dmgcoin transaction input.
type TxIn struct {
...
// NewTxIn returns a new dmgcoin transaction input with the provided
...
// TxOut defines a dmgcoin transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new dmgcoin transaction output with the provided
...
// MsgTx implements the Message interface and represents a dmgcoin tx message.
