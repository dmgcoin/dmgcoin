...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-dmgcoin.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "dmgcoin")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of dmgcoin binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-dmgcoin.conf")
