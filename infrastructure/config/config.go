...
const (
	defaultConfigFilename      = "dmgcoin.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "dmgcoin.log"
	defaultErrLogFilename      = "dmgcoin_err.log"
	...
	sampleConfigFilename    = "sample-dmgcoin.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for dmgcoin.
	DefaultAppDir = util.AppDir("dmgcoin", false)
...
// Flags defines the configuration options for dmgcoin.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in DMGC/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for dmgcoin.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default dmgcoin configuration
...
//
// The above results in dmgcoin functioning properly without any config settings
...
// createDefaultConfig copies the file sample-dmgcoin.conf to the given destination path,
