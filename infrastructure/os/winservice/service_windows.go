...
// long-running dmgcoinMain (which is the real meat of dmgcoin), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start dmgcoinMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once dmgcoin is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("dmgcoinMain-windows", func() {
...
// logServiceStart logs information about dmgcoin when the main server has
