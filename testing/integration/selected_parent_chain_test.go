...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of dmgcoin instances
	dmgcoin1, dmgcoin2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := dmgcoin1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In dmgcoin2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, dmgcoin2)
	}

	// Connect the two dmgcoins. This should trigger sync
	// between the two nodes
	connect(t, dmgcoin1, dmgcoin2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := dmgcoin1.rpcClient.GetVirtualSelectedParentChainFromBlock(
